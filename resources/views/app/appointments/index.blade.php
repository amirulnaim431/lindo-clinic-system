                                ...
                                @foreach ($services as $service)
                                    ...
                                    <label class="block cursor-pointer service-chip {{ $isSelected ? 'is-selected' : '' }}">
                                        ...
                                        <div class="flex items-start justify-between gap-3">
                                            ...
                                            <div class="rounded-xl border border-slate-200 bg-white px-2.5 py-1 text-[11px] font-semibold text-slate-600">
                    ...
                    @if(count($slotOptions))
                        <div class="space-y-4">
                            <div>
                                ...
                                <p class="mt-1 text-xs text-slate-500">Click one slot, then fill in the booking form once.</p>
                            </div>
        ...
        <div class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
            ...
            <div class="divide-y divide-slate-100">
                @forelse ($appointmentGroups as $group)
                    @php
                        $statusValue = $group->status instanceof \BackedEnum
                            ? $group->status->value
                            : (is_string($group->status) ? $group->status : '');
                            ...
                            <form method="POST" action="{{ route('app.appointments.status', $group) }}">
                                ...
                                >
                                    @foreach ($statusOptions as $option)
                                        @php
                                            $optionValue = $option instanceof \BackedEnum
                                                ? $option->value
    ...
    slotButtons.forEach((btn) => {
        btn.addEventListener('click', function () {
            slotButtons.forEach((otherBtn) => {
                const pill = otherBtn.querySelector('.slot-pill');
